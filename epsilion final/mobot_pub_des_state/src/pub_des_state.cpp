...
bool e_stop_stop=false;
int current_mode=999;
std_msgs::Int32 feedback1;
...
void DesStatePublisher::ESTOP_callback(const std_msgs::Bool& message_holder)
{
    e_stop_trigger_ = message_holder.data; 
    e_stop_stop=message_holder.data;
    //if(e_stop_trigger_==false) e_stop_reset_=true;
}
...
/*
  void ESTOP_callback( const std_msgs::bool& message_holder ) 
{ 
...
*/
void DesStatePublisher::initializeSubscriber(){

alarm_subscriber = nh_.subscribe("lidar_alarm",1,&DesStatePublisher::alarmCallback,this);   /// listen to lidar topic
bool e_stop_stop=false;
my_subscriber_object = nh_.subscribe("ESTOP",1,&DesStatePublisher::ESTOP_callback,this);
}
...
void DesStatePublisher::initializePublishers() {
    ...
    feedback_publisher = nh_.advertise<std_msgs::Int32>("Feedback_Topic1", 1);
}
...
bool DesStatePublisher::appendPathQueueCB(mobot_pub_des_state::pathRequest& request, mobot_pub_des_state::pathResponse& response) {
    ...
    if(!path_queue_.empty()) ros::Duration(5.0).sleep();
    return true;
}
...
void DesStatePublisher::pub_next_state() { 
    ...
    switch (motion_mode_) {
        case E_STOPPED:
        if(current_mode==motion_mode_){
        ROS_INFO("estoped");
        current_mode=motion_mode_;
    	}
        //this state must be reset by a service
            desired_state_publisher_.publish(halt_state_);
            if((g_lidar_alarm==false)&&(e_stop_stop==false)) motion_mode_=DONE_W_SUBGOAL;
            break;
        ...
        feedback1.data=0;
                feedback_publisher.publish(feedback1); 
        if(current_mode==motion_mode_){
        ROS_INFO("pursuing_subgoal %d",int(path_queue_.size()));
        current_mode=motion_mode_;
    	}//if have remaining pts in computed traj, send them
        ...
        current_mode=motion_mode_;
    	} //suspended, pending a new subgoal
            ...
            if (!path_queue_.empty()) {
                ...
                // by simply reiterating the last state sent (should have zero vel)
                 ...
                 // record the current system time(absolute time)

//////////// publish the "job done signal for 3 sec"
if(finish_moving)
{
feedback1.data=6;
feedback_publisher.publish(feedback1);
