...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,nav_msgs::Odometry current_des_state_ ,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
alpha_max_=2*alpha_max_;
accel_max_=2*accel_max_;
vec_of_states.clear();
double v=current_des_state_.twist.twist.linear.x;
double vpsi=current_des_state_.twist.twist.angular.z;
double accel = sgn(vpsi) * alpha_max_;
    ...
    double t_linear = v / accel_max_;
    int npts_ramp = round(t_linear / dt_);
  

    double x_des = x_start; //start from here
    double y_des = y_start;
    double speed_des = v;
    double omega_des = vpsi;
...
for (int i = 0; i < npts_ramp; i++) {
    ...
    des_state.twist.twist.linear.x = 0; // make sure 0 speed at end
    vec_of_states.push_back(des_state);


 /////////angular decelerate
double t_angular = vpsi/ alpha_max_;
 npts_ramp = round(t_angular / dt_);
...
for (int i = 0; i < npts_ramp; i++) {
    ...
    //make sure the last state is precisely where requested, and at rest:
    des_state.twist.twist.angular.z = 0; // insist on starting from rest
    vec_of_states.push_back(des_state);
 ...
 alpha_max_=alpha_max_/2;
accel_max_*2*accel_max_/2;
...
void TrajBuilder::build_backward_traj(geometry_msgs::PoseStamped current_pose,std::vector<nav_msgs::Odometry> &vec_of_states)
{
...
double v=0;
    ...
    double x_start = current_pose.pose.position.x;
    double y_start = current_pose.pose.position.y;

    nav_msgs::Odometry des_state;
    des_state.header = current_pose.header; 
    des_state.pose.pose = current_pose.pose; 
   
    double t_linear = 1.4 ;     ///////////rout2  2*(1/2at^2)=1m
    int npts_ramp = round(t_linear / dt_);
    ...
    double y_des = y_start;
    double speed_des = 0;
des_state.pose.pose.orientation = current_pose.pose.orientation;
double psi_des=convertPlanarQuat2Psi(current_pose.pose.orientation);
////////linear decelerate 
...
/////////////Epsilon_modify_//////////////////////Epsilon_modify_//////////////////////Epsilon_modify_//////////////////////Epsilon_modify_/////////
    double backward_flag=end_pose.pose.orientation.x;  ///Quaternion.x use as a flag

    if(backward_flag!=0) build_backward_traj(start_pose,vec_of_states);
