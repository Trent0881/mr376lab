...
/////////////////Epsilon_modify_////////////Epsilon_modify_/////////////Epsilon_modify_/////////////Epsilon_modify_///////////////////////
    ...
    ros::Publisher object_pose = nh.advertise<geometry_msgs::PoseStamped>("Object_Coordinate", 1, true); ///////publish to "Object_Coordinate"
    ...
    feedback.data=2;  //2 means found object
while(!go){ros::spinOnce();} // if not hear from coordinator, do not execute

current_time=ros::Time::now().toSec(); 
while(ros::Time::now().toSec()-current_time<4)
{;}
...
while(1)  // end of the object finder, once found the object, keep publish the object pose to grabber
{
object_pose.publish(g_perceived_object_pose);
ROS_INFO("publish object pose");
while((ros::Time::now().toSec())-current_time<2) //////////// publish the "job done signal for 3 sec"
{
