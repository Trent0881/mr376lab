...
void objectposecallback(const geometry_msgs::PoseStamped& message_holder)
{
object_pickup_poseStamped=message_holder;
object_pickup_poseStamped.pose.position.z-=0.045;
ROS_INFO("received block pose  :x=%f y=%f z=%f",object_pickup_poseStamped.pose.position.x,object_pickup_poseStamped.pose.position.y,object_pickup_poseStamped.pose.position.z);

double psi = 2.0 * atan2(object_pickup_poseStamped.pose.orientation.z, object_pickup_poseStamped.pose.orientation.w);
psi=psi+1.57;
object_pickup_poseStamped.pose.orientation.z = sin(psi / 2.0);
object_pickup_poseStamped.pose.orientation.w = cos(psi / 2.0);
}
...
int main(int argc, char** argv) {
    ...
    ros::Publisher feedback_publisher = nh.advertise<std_msgs::Int32>("Feedback_Topic", 1, true);
    ros::Subscriber Command =nh.subscribe("Command_Topic",1,CommandCallback); /// subscribe to command topic to listen to coordinator
    ros::Subscriber objectpose =nh.subscribe("Object_Coordinate",1,objectposecallback); /// subscribe to command topic to listen to coordinator
 
    ros::Publisher gripper_publisher = nh.advertise<std_msgs::Bool>("/close_gripper", 1, true);
...
////////////////////////////test   gripper//////////////////////////
   ...
   while(((ros::Time::now().toSec())-current_time)<1)
   {
   gripper.data=1;         
...
current_time=ros::Time::now().toSec();
feedback.data=3;
while((ros::Time::now().toSec())-current_time<3) //////////// publish the "job done signal for 3 sec"
{
