...
// Trent Ziemer-created class object to wait for a subscription then proceeed once we get the specified value
class WaitForSub
{
public:
    int data;
    ...
    WaitForSub(ros::NodeHandle *);
    bool waitFor(std::string, int);

private:
    void genericSubCallback(const std_msgs::Int32&);

};

WaitForSub::WaitForSub(ros::NodeHandle * nodeHandle)
{
    nh = nodeHandle;
}

void WaitForSub::genericSubCallback(const std_msgs::Int32& message)
{
    ROS_INFO("Got it!");
    data = message.data;
}

bool WaitForSub::waitFor(std::string subscription_name, int desired_data)
{
	ros::Subscriber generic_sub = nh->subscribe(subscription_name, 1, &WaitForSub::genericSubCallback, this); 
...
int main(int argc, char** argv) {
    ...
    ROS_INFO("Acquire Block Client started (task_client_node)");

    WaitForSub subWaiter(&nh);

    subWaiter.waitFor("some_name", 3);
