...
manip_properties_client_ = nh_.serviceClient<object_manipulation_properties::objectManipulationQuery>("object_manip_query_svc");
  ...
  gripper_publisher = nh_.advertise<std_msgs::Bool>("/close_gripper", 1, true);
...
geometry_msgs::PoseStamped object_pose_stamped1=object_pose_stamped;
object_pose_stamped1.pose.position.z;
...
tf::StampedTransform object_stf1 =
xformUtils.convert_poseStamped_to_stampedTransform(object_pose_stamped1, "object_frame");
...
tf::StampedTransform gripper_stf1;
xformUtils.multiply_stamped_tfs(object_stf1,gripper_wrt_object_stf,gripper_stf1);
...
ROS_WARN("executing plan:approach_pose_ ");
...
if (rtn_val != cartesian_planner::cart_moveResult::SUCCESS) return rtn_val; //return error code
ROS_WARN("executing plan: grasp_pose_");
rtn_val=arm_motion_commander_.execute_planned_path();
